...
function Inflate(temp:GameObject, id:String) {
	...
	//var serialization:String = db[id]; // simulation of server fetch
	var www = new WWW('http://beyondmywall.fe100.net/db/' + id);
	//var www = new WWW('file:///Users/howardstearns/Beyond-My-Wall/server/BMW/public/db/' + id);
    yield www;
   	var serialization = www.text;
	var data:Hashtable = JSON.Parse(serialization);
	Debug.Log('Inflating ' + data['name'] + ' into ' + id);
	var go:GameObject;
...
function Awake () {
	Restore("419d57d1fc5657815cd997fc566910e62a9830f9");
}