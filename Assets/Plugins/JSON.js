...
//	}
	...
	private static function hashtable () : Hashtable
	{
 	...
 	public static function Parse(source:String, revivor) { throw "reviver(key, value) is not implemented."; } // Added. HRS
	public static function Parse( source:String ):Hashtable  // Renamed from ParseJSON. HRS
	{
	...
	public static function HashtableToJSON(hashtable:Hashtable):String
	{
		...
		// This was just keys = hashtable.Keys, but that doesn't preserve order, which made it hard to debug applications. HRS
		var keys = new Array(hashtable.Keys); keys = keys.Sort();
		for(var key in keys)
		{
...
//				retour.Add('"'+key+'" : "'+EscapeString(tempValue.ToString())+'"');
	...
	public static function ObjectToJSON( object:Object ):String
	{
		...
		// This had repeated all the code from the function below. HRS
		return HashtableToJSON(objectForJSON);
	}
