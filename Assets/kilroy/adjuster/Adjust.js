...
function Start() {
	super.Start();
	flipX = Vector3.Dot(axis.right, assembly.up) > 0.2;
...
function doDragging(assembly:Transform, axis:Transform, plane:Transform, hit:RaycastHit) {
	...
	var extent = noFlip(assembly.InverseTransformDirection(newCorner - oppositeCorner) + orthogonalCorner);
	assemblyObj.size(extent);
